...
class EncodeMetarigOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        text = write_metarig(context.active_object, func_name="create", metarig=True)
...
class Q2ECurrentActionOperator(bpy.types.Operator):
    bl_idname = 'gamerig.q2e_current'
...
class Q2EAllActionsOperator(bpy.types.Operator):
    bl_idname = 'gamerig.q2e_all'
...
class Q2EPanel(bpy.types.Panel):
    bl_idname = "GAMERIG_PT_Q2E"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_context = "posemode"
    ...
    def draw(self, context):
        ...
        layout.prop(param, 'q2e_order_list')
        layout.prop(param, 'q2e_convert_only_selected', toggle=True, icon='OUTLINER_DATA_ARMATURE' if param.q2e_convert_only_selected else 'ARMATURE_DATA')

        col = layout.column(align=True)
        row = col.row(align=True)

        row.operator(Q2ECurrentActionOperator.bl_idname, icon='ACTION')
        row = col.row(align=True)
        row.operator(Q2EAllActionsOperator.bl_idname, icon='NLA')
...
register, unregister = bpy.utils.register_classes_factory((
    ...
    RenameBatchOperator,
    MigrateOperator,
    Q2ECurrentActionOperator,
    Q2EAllActionsOperator,
    BoneGroupsUIList,
